...
def train():
    ...
    policy_net = DQN().to(device) # 策略网络（实时更新，同时移动模型到gpu
    target_net = DQN().to(device) # 目标网络（定期同步，同时移动模型到gpu
        ...
        while not done: # 直到游戏结束
            ...
            if random.random() < eps: # 探索
                ...
                with torch.no_grad(): # 禁用梯度
                    q_values = policy_net(torch.tensor(state).unsqueeze(0).float().to(device))
